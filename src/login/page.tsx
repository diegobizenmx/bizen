...
function BillyGreeting({ message }: { message: string }) {
    ...
    <main style={{ 
      ...
      <div style={{
        ...
        <Link href="/" style={{ display: "flex" as const, alignItems: "center" as const, gap: 10, textDecoration: "none", color: "inherit" }}>
          ...
          <strong style={{ fontSize: "clamp(16px, 4vw, 20px)" }}>{brandName}</strong>
        </Link>
        <div style={{ 
          position: "absolute" as const, 
      ...
      <div style={{
        ...
        <div style={{ flex: "0 0 auto", width: "100%", maxWidth: "600px", order: window.innerWidth <= 1024 ? 2 : 1 }}>
          <BillyGreeting message="Hola, Dragón. Aquí es donde tienes que iniciar sesión. Si no tienes una cuenta, da click en 'Crear cuenta'" />
        </div>

        {/* Form Card - RIGHT SIDE */}
        <div style={{ width: "100%", maxWidth: "min(480px, 90vw)", flex: "1 1 480px", order: window.innerWidth <= 1024 ? 1 : 2 }}>
          <Card>
            <div style={{ display: "grid" as const, gap: "clamp(6px, 1.5vw, 8px)", marginBottom: "clamp(12px, 3vw, 16px)", textAlign: "center" }}>
              <h1 style={{ margin: 0, fontSize: "clamp(22px, 5vw, 28px)", color: "#111" }}>Iniciar sesión</h1>
            ...
            <form onSubmit={onSubmit} style={{ display: "grid" as const, gap: 14, maxWidth: 360, margin: "0 auto" }}>
              ...
              <div style={{ display: "flex" as const, justifyContent: "space-between" as const, alignItems: "center" }}>
                ...
                <a href="/forgot-password" style={{ fontSize: 12, color: linkColor, textDecoration: "none", fontWeight: 600 }}>¿Olvidaste tu contraseña?</a>
              </div>
            ...
            {message && (
              <p role="status" style={{ marginTop: 14, textAlign: "center", wordBreak: "break-word", overflowWrap: "anywhere" }}>
                {message}
              </p>
            )}
            ...
            <div style={{ marginTop: 16, display: "grid" as const, gap: 10, maxWidth: 360, marginLeft: "auto", marginRight: "auto" }}>
              ...
              <button
                ...
                style={{ height: 42, borderRadius: 12, border: "1px solid rgba(0,0,0,0.12)", background: "#ffffff", color: linkColor, cursor: "pointer", fontWeight: 700 }}
              >
